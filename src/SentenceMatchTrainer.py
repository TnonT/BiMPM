...
def collect_vocabs(train_path, with_POS=False, with_NER=False):
    ...
    for line in infile:
        ...
        label = items[0]
        sentence1 = re.split(r"\s+",items[1].lower())
        sentence2 = re.split(r"\s+",items[2].lower())
        all_labels.add(label)
        all_words.update(sentence1)
        all_words.update(sentence2)
        if with_POS: 
            all_POSs.update(re.split(r"\s+",items[3]))
            all_POSs.update(re.split(r"\s+",items[4]))
        if with_NER: 
            all_NERs.update(re.split(r"\s+",items[5]))
            all_NERs.update(re.split(r"\s+",items[6]))
    infile.close()
...
def main(FLAGS):
    ...
    # build vocabs 从词向量文件中
    word_vocab = Vocab(word_vec_path, fileformat='txt3')

    best_path = path_prefix + '.best.model' # model path
    char_path = path_prefix + ".char_vocab" # char path
...
def enrich_options(options):
    # if not options.__dict__.has_key("in_format"): # python 2.x version
    s = 'in_format'
    if s not in options.__dict__:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default='../configs/snli.sample.config', help='Configuration file.')
