...
def match_passage_with_question(passage_reps, question_reps, passage_mask, question_mask, passage_lengths, question_lengths,
    ...
    passage_reps = tf.multiply(passage_reps, tf.expand_dims(passage_mask,-1)) # [batch_size, passage_len, feature_dim]
    question_reps = tf.multiply(question_reps, tf.expand_dims(question_mask,-1)) # [batch_size, passage_len, feature_dim]
    ...
    with tf.variable_scope(scope or "match_passage_with_question"):
        ...
        relevancy_matrix = mask_relevancy_matrix(relevancy_matrix, question_mask, passage_mask) #[batch_size, passage_len, question_len]
        ...
        all_question_aware_representatins.append(tf.reduce_max(relevancy_matrix, axis=2,keep_dims=True))   #[batch_size, passage_len, 1]
        all_question_aware_representatins.append(tf.reduce_mean(relevancy_matrix, axis=2,keep_dims=True))  #[batch_size, passage_len, 1]
        dim += 2
        if with_full_match:
            if forward:
                question_full_rep = layer_utils.collect_final_step_of_lstm(question_reps, question_lengths - 1) #[batch_size, feature_dim]
            else:
                question_full_rep = question_reps[:,0,:]

            passage_len = tf.shape(passage_reps)[1]
            question_full_rep = tf.expand_dims(question_full_rep, axis=1) # [batch_size, 1, feature_dim]
