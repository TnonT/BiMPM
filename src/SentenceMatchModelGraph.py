...
class SentenceMatchModelGraph(object):
    ...
    def create_placeholders(self):
        ...
        if self.options.with_char:
            ...
            self.in_question_chars = tf.placeholder(tf.int32, [None, None, None]) # [batch_size, question_len, q_char_len] 每一个batch中的每一个单词包含多少个字母
    ...
    def create_model_graph(self, num_classes, word_vocab=None, char_vocab=None, is_training=True, global_step=None):
        ...
        in_passage_repres = []
        input_dim = 0
        if word_vocab is not None: # 如果单词表存在
        ...
        if options.with_char and char_vocab is not None:
            ...
            self.char_embedding = tf.get_variable("char_embedding", initializer=tf.constant(char_vocab.word_vecs), dtype=tf.float32) # 得到character的embedding
            ...
            in_question_char_repres = tf.reshape(in_question_char_repres, shape=[-1, q_char_len, char_dim]) # [batch_size * question_len,q_char_len, char_dim]
            question_char_lengths = tf.reshape(self.question_char_lengths, [-1])  #[batch_size * question_len]
